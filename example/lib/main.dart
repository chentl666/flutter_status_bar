...
class _MyAppState extends State<MyApp> with WidgetsBindingObserver {
  ...
  didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      if (_useWhiteStatusBarForeground != null)
        FlutterStatusBar.setStatusBarWhiteForeground(
            _useWhiteStatusBarForeground!);
      if (_useWhiteNavigationBarForeground != null)
        FlutterStatusBar.setNavigationBarWhiteForeground(
  ...
  changeStatusColor(Color color) async {
    try {
      await FlutterStatusBar.setStatusBarColor(color, animate: true);
      if (useWhiteForeground(color)) {
        FlutterStatusBar.setStatusBarWhiteForeground(true);
        FlutterStatusBar.setNavigationBarWhiteForeground(true);
        _useWhiteStatusBarForeground = true;
        _useWhiteNavigationBarForeground = true;
      } else {
        FlutterStatusBar.setStatusBarWhiteForeground(false);
        FlutterStatusBar.setNavigationBarWhiteForeground(false);
  ...
  changeNavigationColor(Color color) async {
    try {
      await FlutterStatusBar.setNavigationBarColor(color, animate: true);
    } on PlatformException catch (e) {
      debugPrint(e.toString());
    }
  }
            ...
            children: <Widget>[
                ...
                children: <Widget>[
                  Builder(builder: (BuildContext context) {
                    return ElevatedButton(
                        onPressed: () => FlutterStatusBar.getStatusBarColor()
                                .then((Color? color) {
                  ...
                  ElevatedButton(
                    onPressed: () {
                      ...
                      FlutterStatusBar.setFullscreen(isFullScreen);
                    },
              ...
              Column(
                ...
                children: <Widget>[
                  Builder(builder: (BuildContext context) {
                    return ElevatedButton(
                      onPressed: () => FlutterStatusBar.getNavigationBarColor()
                          .then((Color? color) {
                  ...
                  ElevatedButton(
                    onPressed: () =>
                        FlutterStatusBar.setNavigationBarWhiteForeground(true)
                            .then(
                                (_) => _useWhiteNavigationBarForeground = true),
                    child: Text(
                      'light foreground',
                      style: TextStyle(color: Colors.white),
                    ),
